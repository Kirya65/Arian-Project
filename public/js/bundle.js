...
var displayMap = function displayMap(locations) {
  ...
  locations.forEach(function (loc) {
    ...
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); //Add lines
...
function () {
  var _ref = _asyncToGenerator(
  /*#__PURE__*/
  regeneratorRuntime.mark(function _callee(email, name, password, passwordConfirm) {
    var res;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 5:
            ...
            if (res.data.status == 'success') {
              (0, _alerts.showAlert)('success', "Welome to the Alian App");
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            tp = type === 'password' ? 'updatePassword' : 'updateMe';
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: "http://127.0.0.1:3000/api/v1/users/".concat(tp),
              data: data
            });
...
var stripe = Stripe('pk_test_JYa7dhlEgUK0xdck5KHZqYWp00Xindw25F');
...
var bookBtn = document.getElementById('book-tour'); //DELEGETION
...
if (userDataForm) {
  userDataForm.addEventListener('submit', function (e) {
    ...
    (0, _updateSettings.updateSettings)(form, 'data'); // will recognize form as an object
  });
}
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              ...
              tourId = e.target.dataset.tourId; // e.target это как раз кнопка на которую мы нажимаем. 
              //Вытягиваем tourId с dataset, но так как имена совпадают мы используем деструктуризацию
